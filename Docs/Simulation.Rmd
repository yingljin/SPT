...
bind_rows(
  ...
  facet_wrap(~ksize, nrow = 1, scales = "free_y")+
...
```{r variogram_wt, fig.height=3, fig.width=15}
bind_rows(
...
```{r variogram_stride, fig.height=8, fig.width=6}
bind_rows(
  variogram(str0~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 0),
  variogram(str2~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 2),
  variogram(str4~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 4),
  variogram(str6~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 6),
  variogram(str8~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 8)
) %>%
  ggplot(aes(x=dist, y=gamma, col = as.factor(stride)))+
  geom_line()+
  geom_point()+
...
All five datasets had pretty strong spatial independence. Changing the step size does not seem to affect the spatial independence too much. The direction of effect is also uncertain. 
...
Let's start with one subject (subject 15). I will do the block bootstrap with different block size, and visualize the residual inter-correlation. This turned out to be quite a challenging problem, because to plot the semivariogram, we need each pixel to have a location index. And it is not straight foward to do that, putting the sampled blocks into the original 2D format. 
...
res_list[[k]] %>%
  lapply(function(x){variogram(res~1, locations = ~s1+s2, data = x %>% filter(!is.na(res)) %>% mutate_all(as.numeric))}) %>% 
