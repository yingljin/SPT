...
for(i in stride_vec){
  MAerr_mat <- MA_rand_field_step(kf = 5, ki = 32, stride = i)
  ma_stride[, paste0("str", i)] <- as.vector(MAerr_mat)
}
```
...
```{r variogram_stride, fig.height=3, fig.width=15}
bind_rows(
  variogram(str0~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 0),
  variogram(str1~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 1),
  variogram(str2~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 2),
  variogram(str3~1, locations = ~s1+s2, data = ma_stride) %>% mutate(stride = 3),
