...
- An decreasing function: $w_{ij} \propto exp(max\{d_{ij}\}-d_{ij})$. The $max\{d_{ij}\}$ term is to scale the value of weight down to a range that is comparable to the other functions. 
...
Looks like the monotone weights induced much larger spatial dependence compared to the "smoothed-out" ones, like trigonometric functions.  
