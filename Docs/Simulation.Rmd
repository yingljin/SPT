...
```{r variogram_expwt, fig.height=8, fig.width=5}
df_expwt %>% 
  filter(id==1) %>%
...
In the later bootstrap, there is some troublesome behaviors detected in the decreasing weight function. Specifically, it does not seem to reduce the type I error/increase the power to the same level as in the cases of filter size or exponential weight. I would like to look at the reason for it. 
...
Without scaling, the baseline variation from the decreasing weight is much lower than the rest (about 1/10). To make a fair comparison, let's try if we could scale the weights so that the generated data has the same baseline variation.

Let's try a new weight $w_{ij}'=w_{ij}\frac{k_f}{\sqrt{\sum w_{ij}^2}}$, $k_f$ is filter size (5 in this case).
...
```{r variogram_typewt, fig.height=5, fig.width=5}
df_wt_type %>%
  filter(id==1) %>% 
  mutate(wt_type = factor(wt_type, levels = c("wt_inc", "wt_dec", "wt_u", "wt_sin", "wt_cos"))) %>%
  group_by(wt_type) %>%
...
- Trigonomitric functions have weight that bounces around zero , so there isn't much change over distance. 
